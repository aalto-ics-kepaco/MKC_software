...
obj.L21=0;
for m=1:1:M
	temp(m).hatk= A(Obs(m).id,Obs(m).id,m)*K(Obs(m).id,Obs(m).id,m)*A(Obs(m).id,Obs(m).id,m)';
end
...
for m=1:1:M
	...
	obj.K=obj.K+((normS(K(Obs(m).id,Obs(m).id,m)-temp(m).hatk)^2)/(M*length(Obs(m).id)^2));
     ...
     for l=1:1:size(K,3)
		...
		nObs = length(Obs(l).id);
		if nObs ~= N
			temp2(Obs(l).id,Obs(l).id) =temp2(Obs(l).id,Obs(l).id) + S(m,l)*temp(l).hatk; % relevence part
		else
                	temp2 =temp2 + S(m,l)*temp(l).hatk; % relevence part
		end
            end
     end
     obj.Relevant=obj.Relevant+normS(temp(m).hatk-temp2(Obs(m).id,Obs(m).id))^2/(M*N); 
end
